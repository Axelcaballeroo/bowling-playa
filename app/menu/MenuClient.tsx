...
export default function MenuClient({
  ...
  const [activeSubcategory, setActiveSubcategory] = useState(
    () =>
      menuSubcategories.find((subcategory) => subcategory.section === "bebidas")
        ?.id ||
      menuSubcategories[0]?.id ||
      ""
  );
  ...
  const sectionSubcategories = useMemo(
    ...
    [activeSection, menuSubcategories]
  );
  ...
  }, [activeSection, language, menuProducts, search, selectedSubcategory?.id]);
    ...
    <main className="min-h-screen w-screen max-w-[100vw] overflow-x-hidden bg-[#070609] text-white [background-image:linear-gradient(180deg,#0b080b_0%,#070609_38%,#050506_100%)] [&_*]:box-border">
        ...
        <section className="-mt-6 px-3 pb-2.5 sm:px-4">
          <div className="mx-auto max-w-[720px]">
            <div className="min-w-0 rounded-[22px] border border-white/[0.09] bg-[#111015]/96 px-3 pb-3 pt-3 shadow-[0_18px_55px_rgba(0,0,0,0.38)] backdrop-blur-md sm:px-4 sm:pb-4">
              <Link
                href="/"
                className="mb-2 inline-flex h-7 items-center rounded-full border border-white/[0.08] bg-black/25 px-3 text-[11px] font-black text-zinc-400 transition duration-200 hover:border-[#efd184]/25 hover:bg-[#efd184]/10 hover:text-[#efd184] active:scale-95"
              >
                {language === "es" ? "← Bowling Playa" : "← Back to home"}
              </Link>
      ...
      <section className="relative mx-auto max-w-[720px] px-3.5 pb-12 pt-3.5 sm:px-4 sm:pt-4">
        {showUnavailableNotice && (
          <div className="mb-3 rounded-[18px] border border-[#efd184]/20 bg-[#efd184]/10 px-4 py-3 text-center text-[13px] font-black text-[#efd184]">
